import base64

def encrypt_input(input_str):
    flag = input_str
    flag = encode_with_b64(flag, 345345345)
    flag = reverse_input(flag)
    flag = encode_with_b64(flag, 0)
    flag = unknown(flag, 18)
    return flag

def decrypt_flag(input_str):
    flag = input_str
    flag = unknown(flag, -18)
    flag = decode_with_b64(flag)
    flag = reverse_input(flag)
    flag = decode_with_b64(flag)
    return flag

def check_flag(encrypted_input):
    return encrypted_input == "HL13Oe5cEdBUwYlKM0hONdROENJsFNvz"

def unknown(input_str, something):
    result = []
    for c in input_str:
        if c.isalpha():
            base = ord('A') if c.isupper() else ord('a')
            offset = (ord(c) - base + something) % 26
            if offset < 0:
                offset += 26
            c = chr(base + offset)
        result.append(c)
    return ''.join(result)

def reverse_input(xyz):
    return xyz[::-1]

def encode_with_b64(xyz, integer):
    return base64.b64encode(xyz.encode()).decode()

def decode_with_b64(xyz):
    return base64.b64decode(xyz.encode()).decode()

def main():
    encrypted_flag = "HL13Oe5cEdBUwYlKM0hONdROENJsFNvz"
    flag = decrypt_flag(encrypted_flag)
    print(flag)

if __name__ == "__main__":
    main()
